...
-->
	...
	<xsl:variable name="TypeCodeExclusion" select="/espd:ESPDResponse/ccv:Criterion/cbc:TypeCode[starts-with(., 'CRITERION.EXCLUSION.')]"/><!-- Criterias of the XML that are EXCLUSION (those started with EXCLUSION.) -->
	<rule context="/espd:ESPDResponse">		
		...
		<assert test="( not(string(cbc:ID))=false() )">The element 'espd:ESPDResponse / cbc:ID' is mandatory </assert>
		
		<assert test="( not(string(cbc:IssueDate))=false() )">The element 'espd:ESPDResponse / cbc:IssueDate' is mandatory </assert>
		...
		<assert test="count( cac:AdditionalDocumentReference ) &gt; 0">The ESPDResponse MUST always contain one 'AdditionalDocumentReference' element referring to the Contract Notice (CN) published in TeD (Publications Office). </assert>
	</rule>
