...
-->
	...
	<!-- ESPFRequest elements: elements which values are checked in 'ESPD-codelist-values.cva' -->
	<rule context="cbc:TypeCode | ccv-cbc:JurisdictionLevelCode | cbc:ActivityTypeCode | cbc:LocaleCode | cbc:ContractTypeCode | cbc:CurrencyCode | cbc:ProcurementTypeCode | cbc:RoleCode">
	...
	<!-- ESPFRequest elements: CommonExtensionComponents-2 -->
	<rule context="ext:ExtensionReasonCode">
		<!-- The attributes listID, listAgencyID, listVersionID must be provided and filled in. -->
		<assert test="@listID and @listAgencyID and @listVersionID">One or more of the mandatory attributes (listID, listAgengyID, listVersionID) are not provided </assert>
	</rule>
	
	<!-- ESPFRequest elements: Other -->
	<rule context="cbc:PartyTypeCode | cbc:AddressTypeCode | cbc:AddressFormatCode | cbc:CountrySubentityCode">
		<!-- The attributes listID, listAgencyID, listVersionID must be provided and filled in. -->
		<assert test="@listID and @listAgencyID and @listVersionID">One or more of the mandatory attributes (listID, listAgengyID, listVersionID) are not provided </assert>
	</rule>
	
	<!-- ESPFRequest elements: CommonBasicComponents-2 PENDING TO TAKE A DECISION -->
