...
image::Extended_PaymentOfTaxes_CA_REQUIREMENT(s)_Edition-mock-up.png[Extended 'Payment of Taxes' REQUIREMENT edition, alt="Extended 'Payment of Taxes', REQUIREMENT edition", align="center"]
...
image::Extended_PaymentOfTaxes_EO_REQUIREMENT(s)-mock-up.png[Extended 'Payment of Taxes' EO perspective, alt="Extended 'Payment of Taxes', EO  perspective", align="center"]
...
<4> Notice that type of property used is REQUIREMENT. This entails that: (i) the ESPDRequest is of type EXTENDED (REGULATED ESPDs does not work with REQUIREMENT(s)); (ii) a `cbc:ValueDataType` will be present indicating the type of value the contracting authority uses for this REQUIREMENT (e.g. AMOUNT, INDICATOR, DESCRIPTION, etc.; see the link:https://github.com/ESPD/ESPD-EDM/tree/2.1.1/docs/src/main/asciidoc/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[Code List `ResponseDataType`], the values are the same for contracting authorities REQUIREMENT(s) than for economic operator response values).
