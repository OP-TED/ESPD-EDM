...
For each criteria regarding suitability, the CA could be able to provide: the register name, its URL and the Lots to which each requirement apply.
...
Notice that one set of lots per register: The contracting authority may request that the economic operator is registered in one or multiple registers. Per each one it specifies to which Lots this make sense. This behaviour of “one set of lots per *group of properties* is regular for all those selection criteria where the allocation of lots to a set of 'criterion properties' make sense;
...
Notice that this data structure is quite different from the Regulated ESPD:

. It allows the defines of multiple national subcriteria (to be retrieved from e-Certis): cardinality "0..n" of the SUBCRITERION group;
...
The following XML example illustrates how to solve the more complex situation, when the Lots are specified. If the procurement procedure is not divided into Lots, the XML would be identical, except that all the elements related to Lots would not be present in the XML document.
