...
*Rule*: For the ESDP-EDM it is compulsory use of the value `ESPD_REQUEST` from the Code List link:./dist/cl/ods/ESPD-CodeLists-V2.0.1.ods[_DocRefContentType_]. See also the XML example below on how to specify the ESPD Request. 
...
*Rule*: Optionally use the attribute languageID to indicate the language of the text. Use the Code List link:./dist/cl/ods/ESPD-CodeLists-V2.0.1.ods[_LanguageCodeEU_] for the value of the languageID attribute.
