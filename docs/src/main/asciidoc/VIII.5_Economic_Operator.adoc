...
One relevant aspect is the *Role* of the economic operator. The ESPD-EDM V2.1.1 defines four different roles for the EO. The information to be provided by each role varies:
...
image::Economic_Operator_Mock-up_1_Lots.png[Lots-Extended ESPD, alt="Lots-Extended ESPD", align="center"]
...
The QUESTION(s) relating to the rest of information on the PQS are structured in the criterion `CRITERION.OTHER.EO_DATA.REGISTERED_IN_OFFICIAL_LIST`:
