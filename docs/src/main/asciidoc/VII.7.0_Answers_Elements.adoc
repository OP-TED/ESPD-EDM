...
See the codes used for `cac:ValueDataTypeCode` in the Code List link:./dist/cl/ods/ESPD-CodeLists-V2.0.1.ods[ResponseDataType]. For the different possible values in the response see the XSD diagrams above and the list of the expected elements in `cac:ResponseValue` in the table below.
