...
|===
|*REQUIREMENT*: |**For identifiers, this ESPD V3.0.0 specification requires at least (and always) the mandatory attribute **schemeAgencyID. When identifying parties, like the Economic Operator, the Contracting Authority, etc., the attribute schemeID SHOULD also be used (for this use the code list link:https://github.com/ESPD/ESPD-EDM/blob/3.0.0/docs/src/main/asciidoc/dist/cl/xlsx/[EOIDType]).
|===
