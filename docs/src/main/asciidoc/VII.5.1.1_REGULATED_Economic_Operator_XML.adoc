...
This example contains all the data about an economic operator. Beware that the basic idenification data is placed into the `cac:EconomicOperatorParty` component, whilst the rest of the data (namely for statististical purposes) is structured in the data structures described above.

Compare with the XML example about the economic operator for the SELF-CONTAINED ESPD Response. You will notice important differences, e.g. the Role of the economic operator, number of employees, and other.

The figure below shows how the data about the economic operator looks like in a 'collapsed' *REGULATED* ESPD XML instance (notice that in this example the EO belongs to a group: `cac:EconomicOperatorGroupName`):
