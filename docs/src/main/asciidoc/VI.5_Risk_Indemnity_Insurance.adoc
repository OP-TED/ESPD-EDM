...
[cols="<1, <4"]
|===
|*REQUIREMENT*| Compulsory use of the code list link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.ods[`InsuranceType`] for the SELF-CONTAINED version of the ESPD.
|=== 
...
. In the *SELF-CONTAINED* version the contracting authority can require one minimum amount for multiple insurances, each one of a different type (see the code list link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.ods[`InsuranceType`]);
