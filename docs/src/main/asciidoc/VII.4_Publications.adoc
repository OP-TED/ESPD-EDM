...
|===
|*REQUIREMENT*
|*The economic operator must include in its ESPD Response document a reference to the ESPD Request. For this, the values of the two identifiers `cbc:ID` and `cbc:UUID` of the ESPD Request must be used as the values for the `cbc:ID` and `cbc:UUID` in the ESPD Response (in the reference to ESDP Request).*
|===
