[.text-left]
=== XSD Schema, global view
The ESPD-EDM V02.00.00 uses the UBL-2.2 document named `QualificationApplicationRequest.xsd` XSD Schema. This schema can be found under the folder `dist/xsdrt/maindoc` (or the equivalent documented `xsd` folder).
