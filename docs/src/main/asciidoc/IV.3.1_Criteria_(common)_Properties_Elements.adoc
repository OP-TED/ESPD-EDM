...
*Rule*: Compulsory use of the link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.ods[Code List  'CriterionElementType'].  Possible types are 'CAPTION, REQUIREMENT and QUESTION'. If the type is `CAPTION` or `REQUIREMENT` no answer is expected from the economic operator and therefore the `cbc:ValueDataTypeCode` must be set to `NONE`. Otherwise this value must be set to one of the values defined in the link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.ods[Code List  'ResponseDataType']
...
*Rule*: Compulsory use of the link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.ods[Code List  'ResponseDataType']. Verify that the value *is different to NONE* for properties of type QUESTION.
