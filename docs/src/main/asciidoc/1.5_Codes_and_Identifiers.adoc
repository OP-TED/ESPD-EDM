...
image:Elements_metadata_of_ESPD_codelist.png[Elements and metadata of an ESPD code list,width=600,height=202]
...
However, this ESP-EDM specification also uses the code list link:https://github.com/ESPD/ESPD-EDM/blob/2.1.1/docs/src/main/asciidoc/dist/cl/xlsx/ESPD-CodeLists-V2.1.1.xlsx[EOIDType] with a different purpose, "the identification of the type of scheme used to identify parties, namely Economic Operators" (but should also used to identify the schemes used to identify Buyers, Service Providers, etc.).
...
image:Values_schemeID_Party_Identifiers.png[Values of the schemeID for Party Identifiers,width=600,height=264]
