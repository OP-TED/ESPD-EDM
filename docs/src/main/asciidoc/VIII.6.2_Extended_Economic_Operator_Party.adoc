...
For this case the *EXTENDED* ESPD works identically as the *Basic* ESPD. If the economic operator answers *Yes* the fields about the 'percentage of disabled/disadvantaged workers and the category of handicap to which they belong to should shown, validated or processed (see mock-up for the *Basic* ESPD in section link:#vii-5-1-Basic-economic-operator-party[VIII.5.1 Basic economic operator party]. For this purpose the *Basic* and the *EXTENDED* data structures are identical:
