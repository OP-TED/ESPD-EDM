...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:include href="./inc/BASIC-RootElements-Annotated.xslt"/>
	...
	<xsl:template match="/">
		<QualificationApplicationResponse
			...
			xsi:schemaLocation="urn:oasis:names:specification:ubl:schema:xsd:QualificationApplicationResponse-2 ../xsdrt/maindoc/UBL-QualificationApplicationResponse-2.2.xsd">
	...
	<xsl:template name="createAnswer">
		...
		<xsl:value-of select="$propertyName"/> (PropertyID:<xsl:value-of select="$propertyID"/>) <xsl:text disable-output-escaping="yes"> --&gt;</xsl:text>
		<xsl:variable name="propertyType" select="cbc:TypeCode"/>
		<xsl:if test="$propertyType = 'QUESTION'"> <cac:TenderingCriterionResponse>
				<xsl:call-template name="generateID"/>
				<cbc:ValidatedCriterionPropertyID schemeID="CriteriaTaxonomy"
					schemeAgencyID="EU-COM-GROW" schemeVersionID="2.1.1"><xsl:value-of
						select="cbc:ID"/></cbc:ValidatedCriterionPropertyID>
				<!--cbc:ConfidentialityLevelCode listID="ConfidentialityLevel" listAgencyID="EU-COM-GROW" listVersionID="2.1.1"/-->
	...
	<xsl:template name="createEvidence">
		<cac:Evidence>
			<cbc:ID schemeAgencyID="EU-COM-GROW">EVIDENCE-00001</cbc:ID>
			<cbc:ConfidentialityLevelCode listID="ConfidentialityLevel" listAgencyID="EU-COM-GROW"
				listVersionID="2.1.1">CONFIDENTIAL</cbc:ConfidentialityLevelCode>
	...
	<xsl:template name="createResponseValue">
		...
		<xsl:if test="$propertyDataType != 'PERIOD' and $propertyDataType != 'EVIDENCE_IDENTIFIER'">
			<cac:ResponseValue>
				...
				<xsl:choose>
					<xsl:when
						test="$propertyDataType = 'INDICATOR' or $propertyDataType = 'WEIGHT_INDICATOR' or $propertyDataType = 'ALPHA_INDICATOR'">
						<cbc:ResponseIndicator>true</cbc:ResponseIndicator>
					</xsl:when>
					<xsl:when
						test="$propertyDataType = 'AMOUNT' or $propertyDataType = 'MAXIMUM_AMOUNT' or $propertyDataType = 'MINIMUM_AMOUNT'">
						<cbc:ResponseAmount currencyID="EUR">10000000</cbc:ResponseAmount>
					</xsl:when>
					<xsl:when
						test="$propertyDataType = 'IDENTIFIER' or $propertyDataType = 'ECONOMIC_OPERATOR_IDENTIFIER' or $propertyDataType = 'LOT_IDENTIFIER'">
						<cbc:ResponseID schemeAgencyID="EU-COM-GROW">DUMMY_ID</cbc:ResponseID>
					</xsl:when>
					<xsl:when test="$propertyDataType = 'CODE'">
						<cbc:ResponseCode listAgencyID="EU-COM-GROW" listVersionID="2.1.1"
							listID="PleaseSpecifyTheCorrectOne">DUMMY_CODE</cbc:ResponseCode>
					</xsl:when>
					<xsl:when test="$propertyDataType = 'CODE_COUNTRY'">
						<cbc:ResponseCode listID="CountryCodeIdentifier" listName="ISO-1-ALPHA-2"
							listAgencyID="ISO" listVersionID="1.0"
							>DUMMY_COUNTRY_CODE</cbc:ResponseCode>
					</xsl:when>
					<xsl:when test="$propertyDataType = 'DATE'">
						<cbc:ResponseDate>2014-01-01</cbc:ResponseDate>
					</xsl:when>
					<xsl:when
						test="$propertyDataType = 'DESCRIPTION' or $propertyDataType = 'CERTIFICATION_LEVEL_DESCRIPTION'">
					...
					<xsl:when
						test="$propertyDataType = 'MAXIMUM_VALUE_NUMERIC' or $propertyDataType = 'MINIMUM_VALUE_NUMERIC'">
