...
<xsl:choose>
         ...
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="(cbc:ProcedureCode)">
               ...
               <svrl:text>Information about the procurement procedure MUST be provided ('/cbc:ProcedureCode) when the type of ESPD is Extended.</svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:choose>
	  ...
	  <!--RULE -->
<xsl:template match="cac:TenderingCriterionPropertyGroup[cbc:ID='a53561d5-6614-4dbe-987e-b96f35387f46']/cac:TenderingCriterionProperty[cbc:TypeCode='REQUIREMENT' and cbc:ValueDataTypeCode='LOT_IDENTIFIER']"
                 priority="1001"
                 mode="M7">
