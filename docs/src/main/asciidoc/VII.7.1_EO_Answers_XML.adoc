...
<2> Response value to the global QUESTION. The data type is `cbc:ResponseIndicator`, which is consistent with the expected data value code INDICATOR (from the Code List link:./dist/cl/ods/ESPD-CodeLists-V2.0.2.xlsx[ResponseDataType]).
